...
def get_eigs(trj,chains,chainlength, e_diag, e_off,bbpots, dihedrals,r,pi,J_inter,rij_decay,rij_0,Cz,Dz,Ez,box): #Builds Hamiltonian matrix
    n = chains*chainlength
    H = np.zeros((n,n), dtype = float)
    #electronic contribution
    if Ez == 1:
        for i in range(n):
            H[i,i] = e_diag - bbpots[i] #kT to eV
...
def write_trjwithbbpots(trj_filename, bbpots):
    f_in = open(trj_filename, 'r')
    f_out = open('ovitobbpot.trj','w')
    trjs = bbpots.shape[0]
    n = bbpots.shape[1]
    atomEP = [0, 0, 2, -2]
    for i in range(trjs):
...
def write_trjwithwijmax(trj_filename, bbpots):
    f_in = open(trj_filename, 'r')
    f_out = open('ovitowijmax.trj','w')
    trjs = bbpots.shape[0]
    n = bbpots.shape[1]
    atomEP = [0, 0, 0, 0]
    for i in range(trjs):
...
def write_trjwithwijmax(trj_filename, wij_fileout,bbpots):
    f_in = open(trj_filename, 'r')
    f_out = open('ovitowijmax.trj','w')
    trjs = bbpots.shape[0]
    n = bbpots.shape[1]
